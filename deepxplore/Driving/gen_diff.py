...
for _ in range(args.seeds):
    ...
    angle1 = sess.run(model1.output, feed_dict={input_tensor: gen_img})
    angle2 = sess.run(model2.output, feed_dict={input_tensor: gen_img})
    angle3 = sess.run(model3.output, feed_dict={input_tensor: gen_img})
    if angle_diverged(angle1, angle2, angle3):
        ...
        imageio.imwrite('./generated_inputs/' + 'already_differ_' + str(angle1) + '_' + str(angle2) + '_' + str(angle3) + '.png',
               gen_img_deprocessed)
        continue
    ...
    for iters in range(args.grad_iterations):
        ...
        if angle_diverged(angle1, angle2, angle3):
            ...
            # save the result to disk
            imageio.imwrite('./generated_inputs/' + args.transformation + '_' + str(angle1) + '_' + str(angle2) + '_' + str(
